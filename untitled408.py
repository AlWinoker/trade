# -*- coding: utf-8 -*-
"""
Created on Wed May 24 11:53:49 2023
...
# -*- coding: utf-8 -*-
"""
Created on Mon May 22 08:20:12 2023
...
# -*- coding: utf-8 -*-
"""
Created on Tue May  9 18:44:48 2023
...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 26 23:23:44 2023
...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 26 20:59:26 2023
...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 26 19:03:40 2023
...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 20 00:53:46 2023
...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 19 17:39:36 2023
...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 10 17:15:22 2023
            ...
            if roundOne==0:
             ...
             #   print(klines)
               # klines = klines.drop(columns = [6,7,8,9,10,11])
                ...
                df = klines.copy()
            #    print(df)
              #  df.drop(df.tail(int(1700*7/5)).index,inplace = True)
    ...
    try:
     ...
     #   nsteps = 120
        ...
        if acc>0.5:
            ...
            countacc-=1
        print('The accuracy count is currently',countacc)
        buyprofac.append(buyProf)
        print(np.mean(buyprofac),'buyprofavg')
        countingEverything = countingEverything+1;
        if buyProf<300:
            counting40.append(countingEverything);
            countingEverything=0
        print('40s counted',counting40)
        print(countingEverything);
        print('t40',t40)
        
        
        
        if (counting40!=[]):
            t40.append(counting40)
            counting40=[]
       #     os.remove(r'C:\Users\14015\Downloads\results\_AAPL-sh-1-sc-1-sbd-0-    huber_loss-adam-GRU-seq-20-step-4-layers-4-units-8.h5')
            
            
            
            
      #  price = float(client.get_avg_price(symbol='BTCUSDT')['price'])
        # prices = client.get_all_tickers()
        # for i in prices:
        #     if i['symbol']=='BTCUSDT':
        #         price=float(i['price'])
        price = float(getLast1000Trades('BTCUSDT')[-1]['price'])
        print(price,guess)
        
        timeCheck2 = time.time()
        timeToRun = (timeCheck2-finalTimeData/1000)
        print(timeToRun,'thisthing')
        print('roundone',roundOne,tanky)
        if True:
            print('case 1')
            print(timeToRun)
            if guess>price and timeToRun<10*60:# and timeToRun<30:
                ttrstore=timeToRun
                print('buy')
                print('youarehere')
                cancelAll('BTCUSDT')
                pricetemp=price
                price=round(price,2)
                atemp = float(client.get_asset_balance(asset='USDT')['free'])
                quantity = round((atemp-10)/price,5)
                print('buy')
                buyCrypto('BTCUSDT')
                print('buy')
                ##First number is lookup step, second number is the minutes since the last data entry plus the time taken to calculate the prediction, 60 converts it to minutes
                print('buy')
                time.sleep(3*61)
                price=pricetemp
                price=round(price+price*0.01,2)
                
                # if round(pricetemp,2)>price:
                #     print('error')
                #     break
                # quantity = float(client.get_asset_balance(asset='BTC')['free'])
                
                
                # if round(quantity,5)>quantity:
                #     quantity=round(quantity,5)-0.00001
                ...
                # if quantity>0.0001:
                #     sellordernumber = sellCrypto('BTCUSDT')
                    
                timeCheck2 = time.time()
                timeToRun = (timeCheck2-finalTimeData/1000)/60
                print(timeToRun)
                
                ####the first number in this sleep is equal to the lookup step
                #time.sleep((lookItUp*30-timeToRun)*60)
                atemptest = float(client.get_asset_balance(asset='USDT')['free'])+float(client.get_asset_balance(asset='USDT')['locked'])
                
                print(atemptest,'usdt')
                if ttrstore>8*60 or atemptest<15:
                    while (lookItUp*60-timeToRun)*60>0:
                        #print((lookItUp-timeToRun)*60)
                       # stonks('AAPL')
                        time.sleep(1)
                        timeCheck2 = time.time()
                        timeToRun = (timeCheck2-finalTimeData/1000)/60
              ...
              #  price=pricetemp
                ...
                while (lookItUp*60-timeToRun)*60>0:
                    quantity = float(client.get_asset_balance(asset='BTC')['free'])
                    atemptest2 = float(client.get_asset_balance(asset='BTC')['free'])+float(client.get_asset_balance(asset='BTC')['locked'])
                    print('we loopin')
                    timeCheck2 = time.time()
                    timeToRun = (timeCheck2-finalTimeData/1000)/60
                    if atemptest2>0.00000076*50:
                        #print((lookItUp-timeToRun)*60)
                       # stonks('AAPL')
                     #   time.sleep(1)
        ...
        avgacc.append(acc)
        countloops+=1
        # print('The accumlated average is currrently:',sum(avgacc)/len(avgacc))
        # if buyProf<0 or acc<0.5 or countloops == 4:
        #     print('-----------------------------------------------------------')
        #     countloops=0
        #     if tanky<20 or countloops==4:
        #         roundOne=1
        #         tanky=tanky+1
        #     else:
        #         roundOne=0
        #         tanky=0
        #     print('tanky',tanky,'r1',roundOne)
        now = datetime.datetime.now()
        if now.strftime("%H")==resetTime:
            print('tryingstonks2')
            stonks2('AAPl')
        now = datetime.datetime.now()
